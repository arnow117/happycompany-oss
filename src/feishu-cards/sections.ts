...
export function buildHeader(input: AgentCardInput): El {
  ...
  if (input.subtitle) {
    (header as Record<string, unknown>).subtitle = {
      tag: 'plain_text',
  ...
  (header as Record<string, unknown>).text_tag_list = tags;
  return header;
}
...
function buildRuntimePanel(opts: {
  elementId: string;
...
function collapsiblePanel(opts: CollapsibleOpts): El {
  ...
  if (opts.elementId) {
    (panel as Record<string, unknown>).element_id = opts.elementId;
  }
  return panel;
}
